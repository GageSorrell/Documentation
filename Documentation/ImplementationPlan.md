...
component workbench configurable. It provides Astro documentation websites,
optional React/Vite Storybook at `/storybook/`, reusable React Native Storybook
infrastructure, Effect v4 RC-powered automation, and six installable AI skills.
...
`Documentation` is the published dogfood site, `Website` is the web-package
development sandbox, and both `Application` subdirectories are Android/iOS
Expo Storybook apps.
...
| `@sorrell/docs-astro` | Astro integration, content, Markdown/MDX, routes, article/reference-page integration, Pagefind, redirects, SEO, OG images, and verification. |
...
| `@sorrell/docs-storybook-web` | React/Vite Storybook preset, themes, Autodocs, decorators, and build helpers. |
...
Storybook, repository links, and Vercel. Runtime decoding diagnostics include
the failing path and expected value. The binary supports prompts and
...
Storybook. All commands use Effect-managed subprocesses with structured
concurrency and interruption cleanup.

## Web and native Storybook

The web Storybook uses `@storybook/react-vite`, `@storybook/addon-docs`,
Autodocs, controls, Doc Blocks, official themes, inferred props, and a theme
message bridge at `/storybook/`. Production output lives under
`Distribution/storybook`, is excluded from Pagefind and the sitemap, and is `noindex`.
...
Establish npm workspaces for both Expo apps, `Documentation`, packages, scripts,
and `Website`; centralize the Node 24 and strict TypeScript 6 compiler baseline
...
The full plan also requires API snapshots, Pagefind, Vercel configuration,
release workflows, package-consumer tests, Expo exports/prebuilds, and final
browser accessibility/theme/search/Storybook verification. These are ordered
by `Documentation/DevelopmentMilestones.md` and are intentionally implemented
