...
import type {
    NativeStorybookContextValue,
    NativeStorybookProviderProps,
    SelectionStorage,
    StorySelection
} from "./Types.js";
import { NativeThemeProvider, useNativeTheme } from "./Theme.js";
import { readSelection, writeSelection } from "./Persistence.js";
const SelectionContext = createContext<NativeStorybookContextValue | undefined>(
    undefined
);
...
}) =>
{
    ...
    const [ selection, setSelectionState ] =
        useState<StorySelection>(initialSelection);
    useEffect(() =>
    {
        let active = true;
...
const ProviderBoundary = ({
    ...
    Component === undefined ? (
        ...
        <Component style={ { flex: 1 } }>{children}</Component>
    );
export/** @internal */
const NativeStorybookProvider = ({
    children,
    initialMode = "system",
    initialSelection,
    storage,
    ...
    <ProviderBoundary component={ providers?.GestureHandlerRootView }>
