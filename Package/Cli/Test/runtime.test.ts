...
/**
 * Completion-gate tests for the shared Effect runtime.
 *
 * @file      runtime.test.ts
 * @author    Gage Sorrell <gage@sorrell.sh>
 * @copyright (c) 2026 Gage Sorrell
...
import { Effect, Fiber, FileSystem, Layer, Path, Stdio, Stream, Terminal } from "effect";
...
describe("Effect CLI runtime", () => {
    it.effect("parses flags through the v4 CLI command runner", () =>
        Effect.gen(function*() {
            const parsed: Array<{ readonly count: number; readonly name: string }> = [];
            const command = makeFixtureCommand((input) => Effect.sync(() => parsed.push(input)));

            yield* CliCommand.runWith(command, { renderErrors: false, version: "0.1.0" })([
                "--name",
                "Ada",
                "--count",
                "3"
...
describe("filesystem services", () => {
    it.effect("writes atomically and removes temporary directories on scope close", () =>
        Effect.gen(function*() {
            const fileSystem = yield* DocsFileSystem;
            const path = yield* DocsPath;
            const writer = yield* AtomicWriter;
            const temporaryDirectory = yield* Effect.scoped(Effect.gen(function*() {
                const directory = yield* (yield* TempDirectory).makeScoped({ prefix: "sorrell-docs-cli-" });
                const target = path.join(directory, "config.json");
                yield* writer.writeText(target, "{\"ready\":true}");
                expect(yield* fileSystem.readText(target)).toBe("{\"ready\":true}");
                expect(yield* fileSystem.readDirectory(directory)).toEqual([ "config.json" ]);
                return directory;
            }));
    ...
    it.effect("cleans the temporary file when the atomic rename fails", () =>
        Effect.gen(function*() {
            ...
            const writer = yield* AtomicWriter;
            const temporaryDirectory = yield* Effect.scoped(Effect.gen(function*() {
                const directory = yield* (yield* TempDirectory).makeScoped({ prefix: "sorrell-docs-cli-failure-" });
                const targetDirectory = path.join(directory, "already-a-directory");
                yield* fileSystem.makeDirectory(targetDirectory);
                const outcome = yield* writer.writeText(targetDirectory, "not a directory").pipe(
                    Effect.map(() => ({ _tag: "Right" as const })),
                    Effect.catch((error) => Effect.succeed({ _tag: "Left" as const, left: error }))
                );
                expect(outcome._tag).toBe("Left");
                expect(yield* fileSystem.readDirectory(directory)).toEqual([ "already-a-directory" ]);
                return directory;
            }));
            expect(yield* fileSystem.exists(temporaryDirectory)).toBe(false);
        }).pipe(Effect.provide(docsCliLayer))
...
describe("child-process services", () => {
    it.effect("streams output and returns successful process results", () =>
        Effect.gen(function*() {
            ...
            const lines = yield* Stream.runCollect(runner.streamLines(process.execPath, [
                "-e",
    ...
    it.effect("turns a nonzero child exit into a typed process failure", () =>
        Effect.gen(function*() {
            const runner = yield* CommandRunner;
            const outcome = yield* runner.run(process.execPath, [
                "-e",
    ...
    it.effect("propagates nonzero exits from streamed child output", () =>
        Effect.gen(function*() {
            const runner = yield* CommandRunner;
            const outcome = yield* Stream.runCollect(runner.streamLines(process.execPath, [
                "-e",
    ...
    it.effect("interrupts a scoped child process", () =>
        Effect.gen(function*() {
            const runner = yield* CommandRunner;
            const fiber = yield* runner.run(process.execPath, [
                "-e",
                "setInterval(() => undefined, 1000)"
            ], { forceKillAfter: "100 millis" }).pipe(Effect.forkChild);
