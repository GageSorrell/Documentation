...
describe("automation services", () => {
    it("selects package managers from explicit values and lockfiles", () => {
        expect(selectPackageManager("pnpm@10.0.0", [])).toBe(packageManagerInfo("pnpm"));
        expect(selectPackageManager(undefined, [ "yarn.lock" ])).toBe(packageManagerInfo("yarn"));
        expect(selectPackageManager(undefined, [])).toBe(packageManagerInfo("npm"));
    ...
    it.effect("discovers workspace packages and tracks manifests", () =>
        Effect.gen(function*() {
            ...
            expect(discovered.packages.some((value) => value.name === "@sorrell/docs-core")).toBe(true);

            const temporary = yield* (yield* TempDirectory).makeScoped({ prefix: "sorrell-docs-automation-" });
            const manifestPath = `${temporary}/manifest.json`;
            const manifest = { version: 1 as const, generatedAt: "2026-01-01T00:00:00.000Z", entries: [] };
    ...
    it.effect("validates safe targets and promotes staged directories atomically", () =>
        Effect.scoped(Effect.gen(function*() {
            const fileSystem = yield* DocsFileSystem;
            const temporary = yield* (yield* TempDirectory).makeScoped({ prefix: "sorrell-docs-promotion-" });
            ...
            expect(yield* safeTarget.validateEmpty(target, temporary)).toBe(resolvedTarget);
            yield* (yield* AtomicDirectoryPromotion).promote(staging, target);
            expect(yield* fileSystem.exists(target)).toBe(true);
            yield* fileSystem.writeText(`${target}/generated.txt`, "generated");
            const invalid = yield* safeTarget.validateEmpty(target, temporary).pipe(
                Effect.map(() => Result.succeed(true)),
                Effect.catch((error) => Effect.succeed(Result.fail(error)))
            );
            expect(Result.isFailure(invalid)).toBe(true);
            if (Result.isFailure(invalid)) {expect(invalid.failure).toBeInstanceOf(DocsTargetError);}
        })).pipe(Effect.provide(docsAutomationLayer))
    );
    ...
    it.effect("retries only failures marked retryable", () =>
        Effect.gen(function*() {
            ...
            const transient = yield* retry.run(
                Effect.suspend(() => {
                    attempts += 1;
                    if (attempts < 3) {return Effect.fail(new Error("temporary"));}
            ...
            const nonRetryable = yield* retry.run(
                Effect.suspend(() => {
    ...
    it.effect("runs independent checks with a bound and stages in order", () =>
        Effect.gen(function*() {
            ...
            expect(yield* orchestrator.parallel([ Effect.succeed("a"), Effect.succeed("b") ], 2)).toEqual([ "a", "b" ]);
            expect(yield* orchestrator.sequential([
